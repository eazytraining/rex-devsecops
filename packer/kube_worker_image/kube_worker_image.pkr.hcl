...
locals {
  ...
  merged_tags = merge(
    var.common_tags,
    {
      ...
      "SourceAMI"  = data.amazon-ami.docker_ubuntu.id
    }
  )
}

# Configuration du builder Amazon EBS
source "amazon-ebs" "kube_worker_image" {
  region          = var.aws_region
...
# Définition du build
build {
  name    = "kube_worker_image_build"
  sources = ["source.amazon-ebs.kube_worker_image"]
  
  # Provisioner: Script d'initialisation principal
  provisioner "shell" {
    script          = "../scripts/kube_worker.sh"
